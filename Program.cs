...
namespace UtilityBelt
{
    class Program
    {
        ...
        static void WeatherForecast(IOptions<SecretsModel> options)
        {
            ...
            using (var wc = new WebClient())
            {
                resp = wc.DownloadString(
                    $"http://api.openweathermap.org/data/2.5/weather?q={town}&appid={openWeatherMapApiKey}");
            }

            WeatherRoot wr = JsonSerializer.Deserialize<WeatherRoot>(resp);

            Console.WriteLine();
            Console.WriteLine("Temperature: " + Weather.KtoF(wr.Main.Temp) + "°F or " + Weather.KtoC(wr.Main.Temp) +
                              "°C. Feels like: " + Weather.KtoF(wr.Main.Temp) + "°F or " + Weather.KtoC(wr.Main.Temp) +
                              "°C");
            Console.WriteLine("Wind speed: " + wr.Wind.Speed + " m/s. Air pressure is " + wr.Main.Pressure +
                              "mmHg or " + Math.Round(wr.Main.Pressure * 133.322, 1) + " Pascals.");
        ...
        static void DiscordWebhook(IOptions<SecretsModel> options)
        {
            ...
            {
                Console.WriteLine("Whoops! You dont have a webhook defined in your config!");
                return;
            }
        ...
        static void Covid19()
        {
            Console.WriteLine("");
            Console.WriteLine(
                "Enter the country name to get the information. If you want to see the global information, type \"Global\". For the list of all countries type \"List\": ");
            ...
            {
                ShowCovidInfo("Global", summary.Global.NewConfirmed, summary.Global.TotalConfirmed,
                    summary.Global.NewDeaths, summary.Global.TotalDeaths, summary.Global.NewRecovered,
            ...
            else
            {
                foreach (var countryJson in summary.Countries)
                {
                    ...
                    {
                        ShowCovidInfo(countryJson.Country, countryJson.NewConfirmed, countryJson.TotalConfirmed,
                            countryJson.NewDeaths, countryJson.TotalDeaths, countryJson.NewRecovered,
                            countryJson.TotalRecovered);
                        return;
                    }
                }
            }
        ...
        static void ShowCovidInfo(string countryName, long newConfirmed, long totalConfirmed, long newDeaths,
            long totalDeaths, long newRecovered, long totalRecovered)
        {
        ...
        static void SpaceStationLocation()
        {
            ...
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.WriteLine(
                $@"{spaceStation.Location.Latitude} Latitude and {spaceStation.Location.Longitude} Longitude");
